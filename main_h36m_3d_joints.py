...
def main(opt):
    ...
    net_pred = AttModel.AttModelPerParts(in_features=in_features, kernel_size=kernel_size, d_model=d_model,
                                         num_stage=opt.num_stage,
                                         dct_n=opt.dct_n, parts_idx=parts_idx)
    net_pred.cuda()
...
def run_model(net_pred, optimizer=None, is_train=0, data_loader=None, epo=1, opt=None):
    ...
    for i, (p3d_h36) in enumerate(data_loader):
        ...
        p3d_out_all = net_pred(p3d_src, input_n=in_n, output_n=out_n, itera=itera)  # is_eval=(is_train > 0))
